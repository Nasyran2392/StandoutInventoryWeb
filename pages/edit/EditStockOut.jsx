...
import { stockOutInputs } from '../../formSource'; 

const EditStockOut = ({ inputs }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const stockOutTaskRef = doc(db, "StockOutTasks", id);
        const docSnap = await getDoc(stockOutTaskRef);
        const stockOutTaskData = docSnap.data();
        setData(stockOutTaskData);
        ...
        if (stockOutTaskData && stockOutTaskData.selectedProducts) {
          const selectedProductsData = stockOutTaskData.selectedProducts.reduce((acc, curr) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const stockOutTaskRef = doc(db, "StockOutTasks", id);
      await updateDoc(stockOutTaskRef, updatedData);

      alert("Stock out task updated successfully!");
      setData({});
      setSelectedProducts({});
      navigate("/ListStockOut");
    } catch (error) {
      console.error("Error updating stock out task:", error);
    }
  };

  return (
    <div className="editStockOut">
      <Sidebar />
      <div className="editContainer">
        <Navbar />
        <div className="top">
          <h1>Edit Stock Out Task</h1>
        </div>
